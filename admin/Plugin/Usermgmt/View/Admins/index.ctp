...
$(function(){
	...
	$( "#show_message_div").dialog({
			...
			buttons:{
				...
				$.ajax({
					url: "<?php echo $this->Html->url(array('plugin' => 'usermgmt','controller' => 'admins','action' => 'status_inactive')); ?>",
					data: {'id':user_id},
					type: 'post',
					success: function(r){
						if(r=='error'){
	...
	$( "#delete_user_div").dialog({
			...
			buttons:{
				"<?php echo __('Yes Continue'); ?>": function() {
				$.ajax({
					url: "<?php echo $this->Html->url(array('plugin' => 'usermgmt','controller' => 'admins','action' => 'delete')); ?>",
					data: {'id':user_id},
					type: 'post',
					success: function(r){
						if(r=='error'){
	...
	$( "#suspend_message_div").dialog({
			...
			buttons:{
				"<?php echo __('Yes Continue'); ?>": function() {
				$.ajax({
					url: "<?php echo $this->Html->url(array('plugin' => 'usermgmt','controller' => 'admins','action' => 'change_status')); ?>",
					data: {'id':user_id},
					type: 'post',
					success: function(r){
						if(r=='error'){
						...
						<div class="box-title">
								<h3>
									...
									<?php if(AuthComponent::user('user_role_id')==1){ ?>
										<?php 
									  echo $this->Html->link('<i class="icon-plus icon-white"></i> '.__('Add New Admin',true),array('action'=> 'add'),array('class'=>'btn btn-primary','escape'=>false));	
   ...
   <tbody >
	...
	if( !empty($result) ) {
		...
		<tr class="gallerytr">
			...
			<td  align="left">
				<?php echo $this->Time->timeAgoInWords($records[$model]['last_activity'],array('format' => 'm/d/Y'));  ?>
			</td>
			<td  align="left">
				<?php echo $this->Html->link('<i class="icon-edit"></i>', array('plugin' => 'usermgmt','controller' => 'admins','action' => 'edit',$records[$model]['id']),array('class'=>'btn','rel' => 'tooltip','title'=>__('Edit'),'escape' => false)); ?>
				<?php if($records[$model]['id']!=1){ ?>
				<a href='javascript::void(0)' onclick='return delete_user("<?php echo __('Are you sure you want to delete this admin?'); ?>",this);' id='delete_<?php echo $records[$model]['id']; ?>' class='btn' rel="tooltip" data-placement="top" title="<?php echo __('Delete'); ?>"><i class="icon-remove"></i></a>
				<?php } ?>
			</td>
		</tr>
      <?php
