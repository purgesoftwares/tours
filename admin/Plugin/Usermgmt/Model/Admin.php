...
/**
 * Usermgmt Plugin Admin Model
 *
 * @package Usermgmt
 * @subpackage Usermgmt.models
 */
 //App::uses('Security', 'Utility');
class Admin extends UsermgmtAppModel {
 ...
 */
	...
	public function checkuniquemailadd() {
		...
		$user = $this->MyModel->find('count',array('conditions'=>array('email'=>$this->data['Admin']['email'],'user_role_id !=' => $check_role_id )));
	...
	public function checkuniquemail() {
		...
		$user = $this->MyModel->find('count',array('conditions'=>array('email'=>$this->data['Admin']['email'],'id !='=>$this->data['Admin']['id'] )));
		if($user== 0)
			return true;
		else
			return false; 
 ...
 */
	...
	public function validDate(){
		$month	=	$this->data['Admin']['dob']['month'];
		$day	=	$this->data['Admin']['dob']['day'];
		$year	=	$this->data['Admin']['dob']['year'];
	...
	public function matchpassword(){
		...
		$temppassword	=	$this->data['Admin']['cpassword'];
	...
	public function checkunique(){
		//$this->id;
		if($this->id!=""){
			$userdata	=	$this->find('count',array('conditions'=>array('Admin.email'=>$this->data['Admin']['email'],'Admin.id !='=>$this->id)));
		}else{
			$userdata	=	$this->find('count',array('conditions'=>array('Admin.email'=>$this->data['Admin']['email'])));
		}
