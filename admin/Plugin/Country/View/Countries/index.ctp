...
$(function(){
	...
	$( "#show_message_div").dialog({
			...
			buttons:{
				...
				$.ajax({
					url: "<?php echo $this->Html->url(array('plugin' => 'country','controller' => 'countries','action' => 'status_inactive')); ?>",
					data: {'id':user_id},
					type: 'post',
					success: function(r){
						if(r=='error'){
	...
	$( "#delete_user_div").dialog({
			...
			buttons:{
				"<?php echo __('Yes Continue'); ?>": function() {
				$.ajax({
					url: "<?php echo $this->Html->url(array('plugin' => 'country','controller' => 'countries','action' => 'delete')); ?>",
					data: {'id':user_id},
					type: 'post',
					success: function(r){
						if(r=='error'){							
							...
							<div class="box-title">
								<h3>
									...
									<div class="pull-right">
										<?php 
											echo $this->Html->link('<i class="icon-plus icon-white"></i> '. __('Add New').' '. $singularize,array('plugin'=>'country','controller'=>'countries','action'=> 'add',$dropdown_type),array('class'=>'btn btn-primary','escape'=>false));
									...
									if( !empty($result) ) {
										...
										<tr>
											...
											<td class='hidden-480'>
												...
												<a href='javascript::void(0)' onclick='return delete_user("<?php echo __('Are you sure you want to delete this country?'); ?>",this);' id='delete_<?php echo $records[$model]['id']; ?>' class='btn' rel="tooltip" data-placement="top" title="<?php echo __('Delete'); ?>"><i class="icon-remove"></i></a>
