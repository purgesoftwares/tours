<?php
class LanguagesController  extends LanguageAppController  {
 ...
 */	
	...
	public function index($site_id = 0) {
		...
		$admin_lang[]['Language']	=	array('id'=>1,'name'=>'English','short_code'=>'en','type'=>'admin');
		$admin_lang[]['Language']	=	array('id'=>2,'name'=>'Portuguese','short_code'=>'pt','type'=>'admin');
		$result	=	array_merge($result,$admin_lang);
		// pr($result); die;
	...
	public function edit_expressions($language_id  = 0,$type	=	'app') {
		...
		$short_name = $language_data['Language']['short_code'];
		...
		$extrct_array = $this->extract_language_file($path);
			$extrct_array = array_chunk($extrct_array,400,true);
			// pr(array_keys($extrct_array));
			// $this->data = $extrct_array;
			// pr($this->passedArgs); die;
			
			
			// pr($extrct_array); die;
			$this->set('extrct_array',$extrct_array);
			$this->set('pages',array_keys($extrct_array));
			
			
		
		if(empty($this->data)){
			/* $extrct_array = $this->extract_language_file($path);
			$extrct_array = array_chunk($extrct_array,500,true);
			// pr(array_keys($extrct_array));*/
			$this->data = $extrct_array;
			// pr($this->passedArgs); die;
			...
			$nedited_data = $this->data;
			$edited_data['Language']	=	array();
			$page_id	=	$nedited_data['Language']['page_id'];
			unset($nedited_data['Language']['language_id']);
			unset($nedited_data['Language']['page_id']);
			
			$extrct_array[$page_id]	=	$nedited_data['Language'][$page_id];
			// pr($extrct_array); //die;
			
			if(!empty($extrct_array)){
				foreach($extrct_array as $key=>$exes){
					$edited_data['Language']	=	array_merge($edited_data['Language'],$exes);
				}
			}
			// pr($edited_data['Language']); die;
			/* if(!empty($nedited_data['Language'])){
				foreach($nedited_data['Language'] as $key=>$exes){
					$edited_data['Language']	=	array_merge($edited_data['Language'],$exes);
				}
			} */
			// pr($nedited_data); 
			...
			$this->export_language($short_name,$language_codes[$short_name],$edited_data['Language'],$type);
			
			$temp_models_path = ROOT . DS . $type . DS . 'tmp' . DS. 'cache' . DS. 'models' . DS;
			
			$files = glob($temp_models_path.'*'); // get all file names
			foreach($files as $file){ // iterate files
			  if(is_file($file))
				unlink($file); // delete file
			}
			$temp_persistent_path = ROOT . DS . $type . DS . 'tmp' . DS. 'cache' . DS. 'persistent' . DS;
			
			$files = glob($temp_persistent_path.'*'); // get all file names
			foreach($files as $file){ // iterate files
			  if(is_file($file))
				unlink($file); // delete file
			}
			$temp_views_path = ROOT . DS . $type . DS . 'tmp' . DS. 'cache' . DS. 'views' . DS;
			
			$files = glob($temp_views_path.'*'); // get all file names
			foreach($files as $file){ // iterate files
			  if(is_file($file))
				unlink($file); // delete file
			}
			...
			$this->Session->setFlash( __('Language file successfully edited'), 'success');
				$this->redirect(array('plugin'=>'language','controller'=>'languages','action'=>'edit_expressions',$language_id,$type,'page'=>($page_id+1)));
			$this->data = $extrct_array;
		}
  ...
  function export($site_id,$k,$v) {
	...
	function edit_static_text($language_id = null) {
		  if(!isset($language_id) || $language_id == '' ) {
			 $this->Session->setFlash('Invalid Access.', 'error');
			 $this->redirect(array('action' => 'index'));
		  }
		 
		  $dropdown_type='Contents';
		  $this->loadModel('LanguageText');
			$contents = $this->LanguageText->find('first',array('conditions'=>array('language_id'=>$language_id)));
			
			if(empty($contents)){
				$contents = $this->LanguageText->find('first',array('conditions'=>array('language_id'=>0)));
			}
				$this->LanguageText->id = $contents['LanguageText']['id']; 
			...
			$this->set('singularize', $singularize);
			$this->set('language_id', $language_id);
			$this->set('id', $contents['LanguageText']['id']);
			
			$this->set("model",'LanguageText');
			
		
			if (empty($this->data)) {
				
				$this->data = $this->LanguageText->read();
				// pr($this->data); die;
			 } else {
				 $data = array();
					$data = $this->data;
					
					if($contents['LanguageText']['id']==0){
						$this->LanguageText->create();
					}else{
						$this->LanguageText->id 	=  	$contents['LanguageText']['id'];
					}
					
 					if ($this->LanguageText->save($data,false)) {
						
						$this->Session->setFlash($singularize . ' has been updated.', 'success');
						$this->redirect(array('action' => 'index',$site_id,$language_id));
					}
