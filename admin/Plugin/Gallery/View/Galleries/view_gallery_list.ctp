	...
	$(document).ready(function(){
		...
		$("#create_virtual_tour").on('click',function(){
			...
			$.ajax({
				...
				'type':'post',
				data : $("#GalleryViewGalleryListForm").serialize(),
				dataType : 'json',
				success : function(r){
					alert(r.message);
					$("#copy_tour_url").removeAttr("disable");
					$("#copy_tour_url").on('click',function(){
						 window.prompt("Copy to clipboard: Ctrl+C, Enter", "<?php echo WEBSITE_URL."slideshow/index/".base64_encode($this->data[$model]['id']); ?>");
					});
				}
				});
