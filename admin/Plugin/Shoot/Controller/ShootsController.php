 ...
 */	
	...
	public function index($dropdown_type='Schedule') {		
		...
		$client_list	=	$this->User->find('list',array('conditions'=>array('user_role_id'=>Configure::read("user_role_id.client"),'parent_id'=>$admin_id),'fields'=>array('id','full_name'),'order'=>"last_name asc"));
		...
		if (!empty($this->data)) {
				...
				if($this->{$this->modelClass}->validates()) {
					...
					$products = $data["Shoot"]["product_id"];
					$data["Shoot"]["product_id"] = $data["Shoot"]["product_id"][0];
					// pr($data); die; 
					if ($this->{$this->modelClass}->save($data,false)) {
						$shoot_id = $this->{$this->modelClass}->id;
						$this->loadModel('Gallery');
						if(!isset($data['Gallery']['require_payment_for_access']) || empty($data['Gallery']['require_payment_for_access']))
						$data['Gallery']['require_payment_for_access'] = 0;
						$this->Gallery->save($data,false);
						$gallery_id = $this->Gallery->id;
						$this->{$this->modelClass}->save(array("gallery_id"=>$gallery_id),false);
						$products_data = array();
						foreach($products as $product_id){
							$products_data[] = array("product_id"=>$product_id,"shoot_id"=>$shoot_id);
						}
						$this->loadModel("ShootProduct");
						$this->ShootProduct->saveMany($products_data);
						
						/* save global comments */
						if(isset($data[$this->modelClass]['client_id'])){
							$this->loadModel("UserDetails");
							$user_comment = $this->UserDetails->find('first',array("conditions"=>array("field_name"=>"User.comment","user_id"=>$data[$this->modelClass]['client_id'])));
							$user_comment["UserDetails"]["field_value"] =  $data[$this->modelClass]['global_comment'];
							$this->UserDetails->save($user_comment,false);
						}
						/* Save Invoices */
						if($data['Invoice']['create_invoice']){
							...
							if(isset($data['Shoot']['product_id']) && !empty($data['Shoot']['product_id'])){
								...
								$pcondition = array('id IN'=>$products);
								else
								$pcondition = array('id'=>$products[0]);
								
								$products = $this->Product->find('all',array('conditions'=>$pcondition));
								$products_name = "";
								if(!empty($products)){
									foreach($products as $product){
										...
										if(!empty($product) && $product['Product']['taxable']){
											...
											if(empty($tax)) $tax  = 0;
											$tax_price += round(($product["Product"]['price']*($tax/100)),2);
										}
									}
								}
							...
							$products_name = substr($products_name,0, strlen($products_name)-2);
							// $invoice['tax']	=	round(($data['Shoot']['price']*($tax/100)),2);
							$invoice['tax']	=	$tax_price;
							$invoice['total']	=	$data['Shoot']['price']+$tax_price;
						...
						if(isset($data['Shoot']['product_id']) && !empty($data['Shoot']['product_id'])){
								$this->loadModel("Product");
								// pr($pproducts); die;
								if(is_array($pproducts) && count($pproducts)>1)
								$pcondition = array('id IN'=>$pproducts);
								else
								$pcondition = array('id'=>$pproducts[0]);
								
								$pproducts = $this->Product->find('all',array('conditions'=>$pcondition));
								$products_name = "";
								if(!empty($pproducts)){
						...
						/* Send shoot booked email */
							...
							$shoot_title   	=   $data[$this->modelClass]['title'];
							$shoot_date   	=   $data[$this->modelClass]['date'];
							$shoot_size   	=   $data[$this->modelClass]['size'];
							$shoot_price   	=   $data[$this->modelClass]['price'];
							$product	   	=   $product_list[$data[$this->modelClass]['product_id']];
							$photographer  	=   $photographer_list[$data[$this->modelClass]['photographer_id']];
							
							
							if(isset($invoice) && $invoice['recipient_id']!= $invoice['client_id']){
								$this->loadModel("Contact");
								$contact = $this->Contact->findById($invoice['recipient_id']);
								...
								$client = $this->User->find("first",array("conditions"=>array("User.id"=>$data[$this->modelClass]['client_id']),'fields'=>array("first_name","last_name","email")));
							...
							if(!isset($products_name) || $products_name==''){
								$products_name = $product;
							}
							$rep_Array = array($shoot_title,$shoot_date,$shoot_size,$shoot_price,$products_name,$photographer,$first_name,$last_name); 
	...
	function add_product($num = 1){
		$this->loadModel("Product");
		$product_list = $this->Product->find('list',array('fields'=>array('id','name'),'conditions'=>array('admin_id'=>$this->Auth->user('id'))));
		$this->set('product_list', $product_list);
		$this->set('num',$num);
		...
		if($this->request->is('Ajax')){
		 ...
		 if($this->data['date'] != null && ($this->Auth->user('user_role_id')==1)){
			$this->{$this->modelClass}->belongsTo = array(
			 ...
			 $shoots = $this->{$this->modelClass}->find('all',array('conditions'=>array('date'=>$this->data['date'],'Client.parent_id'=>$this->Auth->user('id'))));
			 if(!empty($shoots)){
...
require_once(APP . 'Vendor' . DS . 'dompdf' . DS . 'dompdf_config.inc.php');
		 ...
		 if($this->data['id'] != null && ($this->Auth->user('user_role_id')==1)){
			 $this->{$this->modelClass}->id = $this->data['id'];
				...
				// if(1){
					...
					/* Send shoot editing list status done email */
							$this->{$this->modelClass}->belongsTo = array("Client"=>array("className"=>"Usermgmt.Client","foreignKey"=>"client_id"));
							$shoot = $this->{$this->modelClass}->findById($this->data['id']);
							// pr($shoot); die;
							$this->loadModel('EmailTemplate');
							$this->loadModel('Setting');
							$this->loadModel('EmailAction');
							...
							$reg_succ_id	=	Configure::read('global_ids.email_template.editing_status_done');
							...
							//SHOOT_TITLE,SHOOT_TIME
							$shoot_title   	=   $shoot[$this->modelClass]['title'];
							$shoot_date   	=   $shoot[$this->modelClass]['date']." ".$shoot[$this->modelClass]['hour'].":".$shoot[$this->modelClass]['min'].$shoot[$this->modelClass]['meridian'];
							
							// $email   		=   $shoot["Client"]["email"];
							$email   		=   Configure::read('Site.editing_list_email');
							
							$rep_Array = array($shoot_title,$shoot_date,$email); 
	...
	//SHOOT_TITLE,SHOOT_TIME,FIRST_NAME,LAST_NAME,RELEASE_DATE,PAYMENT
			...
			$due_shoots = $this->{$this->modelClass}->find('all',array('conditions'=>array('Shoot.date'=>date('m')."-".(date('d')+1)."-".date('Y'),'Invoice.status'=>0,"Invoice.send_reminder"=>1 )));
			
		 if(!empty($due_shoots)){
		 // pr($due_shoots); die;
			foreach($due_shoots as $shoot){
			
				/* Send shoot invoice reminder email */
							...
							$reg_succ_id	=	Configure::read('global_ids.email_template.invoice_reminder');
							...
							//SHOOT_TITLE,SHOOT_TIME,FIRST_NAME,LAST_NAME,RELEASE_DATE,PAYMENT
							$shoot_title   	=   $shoot[$this->modelClass]['title'];
							$shoot_time   	=   $shoot[$this->modelClass]['date']." ".$shoot[$this->modelClass]['hour'].":".$shoot[$this->modelClass]['min'].$shoot[$this->modelClass]['meridian'];
							
							$first_name 	=	$shoot["Client"]['first_name']; 
							$last_name 		=	$shoot["Client"]['last_name']; 
							$release_date	=	isset($shoot["Gallery"]['date'])?$shoot["Gallery"]['date']:$shoot[$this->modelClass]['date']; 
							$payment		=	$shoot["Invoice"]['payment']-$shoot["Invoice"]['discount_amount']-$shoot["Invoice"]['paid']; 
							$email   		=   $shoot["Client"]["email"];
							
							if($shoot["Invoice"]['recipient_id']!= $shoot["Invoice"]['client_id']){
								$this->loadModel("Contact");
								$contact = $this->Contact->findById($shoot["Invoice"]['recipient_id']);
	...
	//SHOOT_TITLE,SHOOT_TIME,FIRST_NAME,LAST_NAME,RELEASE_DATE,PAYMENT
			...
			$due_shoots = $this->{$this->modelClass}->find('all',array('conditions'=>array('Gallery.date'=>date('m')."-".date('d')."-".date('Y') )));
		 ...
		 // pr($due_shoots); die;
			...
			foreach($due_shoots as $shoot){
				...
				if($shoot["Gallery"]["meridian"] == date("a") && $shoot["Gallery"]["hour"] == date("g")-5){
				/* Send shoot gallery release email */
							...
							if($shoot["Gallery"]['recipient']!= $shoot["Gallery"]['client_id']){
								$this->loadModel("Contact");
								$contact = $this->Contact->findById($shoot["Gallery"]['recipient']);
	...
	function send_invoice_past_due_email() {
	//SHOOT_TITLE,SHOOT_TIME,FIRST_NAME,LAST_NAME,RELEASE_DATE,PAYMENT
			...
			$due_shoots = $this->{$this->modelClass}->find('all',array('conditions'=>array("UNIX_TIMESTAMP(STR_TO_DATE(Invoice.due_date, '%m-%d-%Y')) <"=>time(),'Invoice.status'=>0 )));
			
		 // pr($due_shoots); die;
		 if(!empty($due_shoots)){
			$pre_days = Configure::read("Site.time_invoice_past_due_email");
			foreach($due_shoots as $shoot){
				
				$due_date = explode('-',$shoot["Invoice"]['due_date']);
				
				$time_diff = strtotime("now")-strtotime($due_date[1]."-".$due_date[0]."-".$due_date[2]);
				echo $due_days = floor($time_diff/86400); 
				
				
				if(!($due_days%$pre_days)){
				/* Send shoot invoice reminder email */
							...
							$reg_succ_id	=	Configure::read('global_ids.email_template.invoice_past_due_email');
