...
$(function(){
	...
	$( "#show_message_div").dialog({
			...
			buttons:{
				...
				$.ajax({
					url: "<?php echo $this->Html->url(array('plugin' => 'transaction','controller' => 'transactions','action' => 'active_status')); ?>",
					data: {'id':user_id},
					type: 'post',
					success: function(r){
						if(r=='error'){
								...
								<table class="table table-hover table-nomargin dataTable table-bordered">
									...
									if( !empty($result) ) {
										...
										<tr>
											...
											<td><?php echo ($records[$model]['status'])?(($records[$model]['paid'])?"Paid":"Active"):"Cancelled"; ?></td>
											<td ><?php 
											...
											<td>
												...
												<a style ="margin-top:5px;" href='javascript::void(0)' onclick='return show_message("<?php echo __('Are you sure you want to active this transaction?'); ?>",this);' id='complete_<?php echo $records[$model]['id']; ?>' class="btn" rel="tooltip" title="<?php echo __('Active'); ?>"><i class="icon-ok-sign icon-white"></i></a>
