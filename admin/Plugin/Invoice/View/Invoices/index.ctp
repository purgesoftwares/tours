							...
							<div class="box-content nopadding">
								<table class="table table-hover table-nomargin dataTable table-bordered">
									...
									if( !empty($result) ) {
										...
										//pr($records);
										$due_date = explode('-',$records[$model]['due_date']);
										// echo $due_date[1]."-".$due_date[0]."-".$due_date[2];
										?>
										<tr>
											<td><?php echo $records[$model]['due_date'];?></td>
											<td><?php echo $this->Html->link($records[$model]['title'],array('action'=>'view',$records[$model]['id']));?></td>
											...
											<td><?php echo  '$'.$records[$model]['total']; ?></td>
											<td><?php echo $this->Html->link('<i class="icon-edit"></i>', array('action' => 'edit',$client_id,$records[$model]['id']),array('class'=>'btn','rel' => 'tooltip','title'=>__('Edit'),'escape' => false)); ?></td>
