...
<button class="btn" id="add_new_shoot" style="display:none;position:relative;z-index:9999;">+ Add New Shoot
	</button>
</div>
<div id="navigation">
...
<script>
$(function(){
		$.datepicker.setDefaults({dateFormat: 'dd-mm-yy',changeMonth: true,
		...
		$("select[id$=County]").live('change',function(){
			$.ajax({
				'url':"<?php echo $this->Html->url(array('plugin'=>false,'controller'=>'pages','action'=>'get_locals'));?>/"+$(this).val(),
				'type':'post',
				'dataType':'json',
				'success':function(local_list){
					options	="<option value=''><?php echo __('Select Local');?></option>";
					$.each(local_list, function(value,name){
						options+="<option value='"+value+"'>"+name+"</options>";
					});
					$("select[id$=Local]").html(options);
				}
			});
			
		});
		
		$("select[id$=BrandCategory]").live('change',function(){
		
			$.ajax({
				'url':"<?php echo $this->Html->url(array('plugin'=>false,'controller'=>'pages','action'=>'get_sub_categories'));?>/"+$(this).val(),
				'type':'post',
				'dataType':'json',
				'success':function(local_list){
					options	="<option value=''><?php echo __('Select Sub Category');?></option>";
					$.each(local_list, function(value,name){
						options+="<option value='"+value+"'>"+name+"</options>";
					});
					$("select[id$=SubCategory]").html(options);
				}
			});
