...
//pr($_SESSION);
		...
		$this->Shoot->belongsTo = array(
										"Photographer"=>array('className'=>'Usermgmt.Photographer','foreignKey'=>'photographer_id'),
										"Gallery"=>array("className"=>"Gallery.Gallery","foreignKey"=>"gallery_id"),
										"Client"=>array("className"=>"Usermgmt.Client","foreignKey"=>"client_id")
										);
		/* $this->Shoot->hasOne = array(
												"Invoice"=>array("className"=>"Invoice.Invoice","foreignKey"=>"shoot_id")
												);
		 */
		$due_shoots = $this->Shoot->find('all',array('conditions'=>array('Gallery.date'=>date('m-d-Y'),'Client.parent_id'=>$user_ids/* ,'status !='=>4 */)));
