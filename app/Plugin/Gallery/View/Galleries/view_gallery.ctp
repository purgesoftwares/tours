      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
							...
							<div class="box-content nopadding" >
									...
									<div class="span7" >
									  ...
									  <div class="control-group <?php echo ($this->Form->error($model.".alt_agent_phone"))? "error":"";?>">
									   ...
									   <div class="form-actions">
										...
										<?php echo $this->html->link(__("Copy Tour URL", true),'javascript:void(0);',array("class"=>"btn btn-primary",'disabled'=>'disabled','id'=>'copy_tour_url')); ?>
										<?php echo $this->html->link(__("Copy Non-Branded Tour URL", true),'javascript:void(0);',array("class"=>"btn btn-primary",'disabled'=>'disabled','id'=>'copy_unbrand_tour_url','style'=>"margin-top:10px")); ?>
										&nbsp; <i class="icon-info-sign" id="popoverData" href="#" data-content='You must click on "Create Virtual Tour" at least once to make URL available. Click again to save any changes.' rel="popover" data-placement="bottom" data-trigger="hover" ></i>
								...
								echo $this->Form->create("Rating",array('url'=>array("controller"=>"galleries",'action'=>'rating'),"class"=>"form-validate form-horizontal"));
									echo $this->Form->hidden('gallery_id',array('value'=>$this->data[$model]['id']));
									echo $this->Form->hidden('client_id',array('value'=>$this->data[$model]['client_id']));
								?>
									<style>
									.caption{display:none!important;}
									i.glyphicon-minus-sign{display:none!important;}
									</style>
									<div class="span4" >
									  ...
									  <div class="control-group " style="margin-bottom:0px;">
									   ...
									   <div style="margin-left:-150px;">
										...
										<div class="controls">
										<?php echo $this->Form->number("Rating".".rating",array('class'=>'rating','id'=>'rating',"min"=>1, "max"=>5, "step"=>.5, "data-size"=>"sm","data-rtl"=>"false","showClear"=>0,"showCaption"=>0,'value'=>5)); ?>
										<span class="help-inline"><?php echo $this->Form->error("Rating".".rating",array("wrap"=>false)); ?></span>
										</div>
										</div>
									  </div>
									  <script>
									  // with plugin options
										// $("#rating").rating(['min'=>1, 'max'=>5, 'step'=>.5, 'size'=>'sm','showClear'=>false,'showCaption'=>false]);
									  </script>
									   <div class="control-group <?php echo ($this->Form->error("Rating".".comment"))? "error":"";?>" style="margin-bottom:0px;">
									   
									   <div style="margin-left:-80px;">
									   <?php echo $this->Form->label("Rating".".comment", __('Comments').": ",array("class"=>"control-label",'style'=>'font-weight:bold;clear:both;margin-left:20px;')); ?><br/>
										<div class="controls" style="clear:both;margin-left:90px;">
										<?php echo $this->Form->textarea("Rating".".comment",array('class'=>'',"data-rule-required"=>true,'rows'=>10)); ?><span class="help-inline"><?php echo $this->Form->error("Rating".".comment",array("wrap"=>false)); ?></span>
										</div>
										</div>
										
										<div class="controls" style="clear:both;margin-left:10px; margin-top:10px;">
										 <span style="float:left;"><?php echo $this->Form->checkbox("Rating".".send_to_photographer",array('class'=>'','value'=>1,'style'=>"float:left;")); ?> Send notification to assigned photographer?</span><span class="help-inline"><?php echo $this->Form->error("Rating".".send_to_photographer",array("wrap"=>false)); ?></span>
										</div>
										</div>
										
									   <div class="form-actions" style="margin-left:-90px;margin-top:10px;">
										<div class="input">
										<?php echo $this->Form->submit(__("Submit", true),array("class"=>"btn btn-primary",'id'=>'create_virtual_tour')); ?>
										</div>
									  </div>
									  </div>
		...
		$("#create_virtual_tour").on('click',function(){
			...
			$.ajax({
				...
				success : function(r){
					...
					$("#copy_unbrand_tour_url").removeAttr("disabled");
					...
					$("#copy_unbrand_tour_url").on('click',function(){
						 window.prompt("Copy to clipboard: Ctrl+C, Enter", "<?php echo substr(strstr(WEBSITE_URL,"//"),2,strlen(WEBSITE_URL)-1)."slideshow/nonBranded/".base64_encode($this->data[$model]['id']); ?>");
					});
