...
/**
 * Usermgmt Plugin Client Model
 *
 * @package Usermgmt
 * @subpackage Usermgmt.models
 */
 //App::uses('Security', 'Utility');
class Client extends UsermgmtAppModel {
 ...
 */
	...
	function clientPasswordValidate() {
			$validate1 = array(
				'password'=> array(
					'mustNotEmpty'=>array(
						'rule' => 'notEmpty',
	...
	public function checkuniquemailadd() {
		...
		$user = $this->MyModel->find('count',array('conditions'=>array('email'=>$this->data['Client']['email'],'user_role_id !=' => $check_role_id )));
	...
	public function checkuniquemail() {
		...
		$user = $this->MyModel->find('count',array('conditions'=>array('email'=>$this->data['Client']['email'],'id !='=>$this->data['Client']['id'] )));
		if($user== 0)
			return true;
		else
			return false; 
 ...
 */
	...
	public function validDate(){
		$month	=	$this->data['Client']['dob']['month'];
		$day	=	$this->data['Client']['dob']['day'];
		$year	=	$this->data['Client']['dob']['year'];
	...
	public function matchpassword(){
		...
		$temppassword	=	$this->data['Client']['cpassword'];
	...
	public function checkunique(){
		//$this->id;
		if($this->id!=""){
			$userdata	=	$this->find('count',array('conditions'=>array('Client.email'=>$this->data['Client']['email'],'Client.id !='=>$this->id)));
		}else{
			$userdata	=	$this->find('count',array('conditions'=>array('Client.email'=>$this->data['Client']['email'])));
		}
