...
/**
 * Usermgmt Plugin Employee Model
 *
 * @package Usermgmt
 * @subpackage Usermgmt.models
 */
 //App::uses('Security', 'Utility');
class Employee extends UsermgmtAppModel {
 ...
 */
	...
	//public $virtualFields  =  array('ron_total' => "SELECT SUM(profit) FROM `transactions` WHERE `completed` = '1' AND `currency` = 'RON' AND `employee_id`= Employee.id ",
 ...
 */
	...
	function employeePasswordValidate() {
			$validate1 = array(
				'password'=> array(
					'mustNotEmpty'=>array(
						'rule' => 'notEmpty',
	...
	public function checkuniquemailadd() {
		...
		$user = $this->MyModel->find('count',array('conditions'=>array('email'=>$this->data['Employee']['email'],'user_role_id !=' => $check_role_id )));
	...
	public function checkuniquemail() {
		...
		$user = $this->MyModel->find('count',array('conditions'=>array('email'=>$this->data['Employee']['email'],'id !='=>$this->data['Employee']['id'] )));
		if($user== 0)
			return true;
		else
			return false; 
 ...
 */
	...
	public function validDate(){
		$month	=	$this->data['Employee']['dob']['month'];
		$day	=	$this->data['Employee']['dob']['day'];
		$year	=	$this->data['Employee']['dob']['year'];
	...
	public function matchpassword(){
		...
		$temppassword	=	$this->data['Employee']['cpassword'];
	...
	public function checkunique(){
		//$this->id;
		if($this->id!=""){
			$userdata	=	$this->find('count',array('conditions'=>array('email'=>$this->data['Employee']['email'],'id !='=>$this->id)));
		}else{
			$userdata	=	$this->find('count',array('conditions'=>array('email'=>$this->data['Employee']['email'])));
		}
