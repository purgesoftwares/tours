			...
			<div class="row-fluid">
				<div class="span5" >
					<div class="control-group <?php echo ($this->Form->error($model.".password"))? "error":"";?>"> 
						<?php echo $this->Form->label($model.".password", __('Email').": *",array("class"=>"control-label")); ?>
...
<script>
	...
	$("#save_new_client_button").on('change',function(){
		...
		$.ajax({
				...
				'type':'post',
				'data':{'first_name':$("#ShootFirstName").val(), 'last_name':$("#ShootLastName").val(), 'telephone':$("#ShootTelephone").val(), 'email':$("#ShootEmail").val(), 'email':$("#ShootPassword").val()},
				'dataType':'json',
				'success':function(response){
					if(response.status){
						options	="";
