 ...
 */
	...
	public function login() {
		...
		if(!empty($this->data)){	
			...
			// echo $this->Auth->password($this->data[$this->modelClass]['password']);
			// pr($this->data); die;
		...
		if ($this->Auth->user()) {
			...
			if($this->Auth->user('user_role_id')){ 
				...
				$userdata =  $this->{$this->modelClass}->find('first',array('conditions'=>array('User.id'=>$this->Auth->user('id'))));
				//pr($userdata); die;
	...
	$this->layout	=	'home';
		...
		if(!empty($this->data)) {		
			...
			$email_data	=	$this->{$this->modelClass}->find('first',array('conditions'=>array($this->modelClass.'.email'=>$this->data[$this->modelClass]['email'])));
	...
	function resetpassword($validate_string=null){
		...
		$this->layout	=	'home';
		$msg	=	'';
		$userInfo	=	$this->{$this->modelClass}->find('first',array('conditions'=>array($this->modelClass.'.forgot_password_validate_string'=>$validate_string)));	
		//pr($userInfo); die;
		if(!empty($userInfo)){
			if(!empty($this->data)) {
				$this->{$this->modelClass}->set($this->data);	
