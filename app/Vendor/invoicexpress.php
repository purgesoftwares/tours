...
class invoicexpress{
	...
	function client_create($client_data=array(),$send_options=1){
		...
		$create_client_array	=	array(
										'name'=>$client_data['User']['name'],
										'code'=>$client_data['User']['id'],
										'email'=>$client_data['User']['email'],
										'address'=>$client_data['User']['address'],
		...
		$curl_response		=	$this->convertxml_to_array($this->init_request($url,$client_xml));
	...
	function send_invoice($invoice_id,$message_data=array()){
		
		
		$message_xml			=	$this->array_to_xml($message_data,'message');
		// create a url
		$url					=	$this->config['url']['main']."/invoice/".$invoice_id."/email-invoice.xml?api_key=".$this->config['api']['api_key'];
		// initiate curl request
		$curl_response			=	$this->init_request($url,$message_xml,'PUT');
		return $curl_response;	
	}	
}
?>