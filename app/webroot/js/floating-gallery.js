...
var dragDropDiv;
var insertionMarker;
var insertionMarkerXMinus = 0;
var insertionMarkerYMinus = 0;
...
{		
  ...
  // returnValue -= insertionMarkerXMinus;
  return returnValue;
}
...
{	  
  ...
  // returnValue -= insertionMarkerYMinus;
  return returnValue;
}
	
function selectImage(e)
{	
	// alert(document.getElementById("Sort_by").value);
	if(document.getElementById("Sort_by").value!="manual")
						window.location = manual_url;
...
function startMoveTimer(){
	...
	if(moveTimer==10){
		...
		dragDropDiv.style.top = tmpTop - insertionMarkerYMinus + 'px';
		dragDropDiv.style.left = tmpLeft - insertionMarkerXMinus + 'px';
...
function dragDropMove(e)
{
	...
	dragDropDiv.style.top = topPos - insertionMarkerYMinus + 'px';
	dragDropDiv.style.left = leftPos - insertionMarkerXMinus + 'px';
	...
	for(var prop in divXPositions){
		if(divXPositions[prop]/1 < leftPos/1 && (divXPositions[prop]/1 + divWidth[prop]*0.7)>leftPos/1 && divYPositions[prop]/1<topPos/1 && (divYPositions[prop]/1 + divWidth[prop])>topPos/1){
			...
			insertionMarker.style.top = divYPositions[prop] + offsetY - insertionMarkerYMinus -0 + 'px';
			insertionMarker.style.left = divXPositions[prop] + offsetX - insertionMarkerXMinus -0 + 'px';
